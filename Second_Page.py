...
@st.cache_data
def load_data():
    df = pd.read_csv("Train_data.csv", dtype={'train_number': str})
...
def show_page2():
    ...
    if selected_station:
        st.markdown(f'<div class="station-badge">📍 Showing data for: {selected_station}</div>', unsafe_allow_html=True)
        station_df = df[(df["source_station"] == selected_station) | (df["destination_station"] == selected_station)].copy()
    else:
        st.markdown('<div class="station-badge">📍 All Stations</div>', unsafe_allow_html=True)
        station_df = df.copy()
    ...
    total_routes  = station_df.groupby(["source_station", "destination_station"]).ngroups

    s1, s2, s3, s4 = st.columns(4)
    s1.markdown(f'<div class="stat-card teal-card"><div class="number">{total_trains}</div><div class="label">Unique Trains</div></div>', unsafe_allow_html=True)
    s2.markdown(f'<div class="stat-card green-card"><div class="number">₹{avg_fare:.0f}</div><div class="label">Avg Fare</div></div>', unsafe_allow_html=True)
    s3.markdown(f'<div class="stat-card purple-card"><div class="number">{avg_duration:.1f} hrs</div><div class="label">Avg Duration</div></div>', unsafe_allow_html=True)
    s4.markdown(f'<div class="stat-card orange-card"><div class="number">{total_routes}</div><div class="label">Routes</div></div>', unsafe_allow_html=True)

    st.divider()

    # ── 💰 Fare Trends by Class — Line Chart ─────────────
    st.markdown('<div class="section-title">💰 Fare Trends by Class</div>', unsafe_allow_html=True)
    sdf = station_df.copy()
    sdf["dist_bin"] = pd.cut(
        sdf["distance_km"],
        bins=[0, 100, 250, 500, 750, 1000, 1500, 2000, 5000],
        labels=["0-100","100-250","250-500","500-750","750-1k","1k-1.5k","1.5k-2k","2k+"]
    )
    fare_line = sdf.groupby(["dist_bin","class_type"])["fare"].mean().reset_index()
    ...
    st.markdown('<div class="section-title">🏙️ Top Stations</div>', unsafe_allow_html=True)
    ts1, ts2 = st.columns(2)

    with ts1:
        st.markdown("**📤 Top Departure Stations**")
        top_src = station_df["source_station"].value_counts().head(8).reset_index()
    ...
    with ts2:
        ...
        top_dst = station_df["destination_station"].value_counts().head(8).reset_index()
    ...
    st.markdown('<div class="section-title">🗺️ Busiest Routes</div>', unsafe_allow_html=True)
    ...
    if arc_data:
        ...
        arc_df["width"] = ((arc_df["count"] / max_c) * 8).clip(lower=1).astype(float)

        arc_layer = pdk.Layer(
            "ArcLayer",
            data=arc_df.to_dict('records'),
        ...
        deck = pdk.Deck(
            ...
            tooltip={
                ...
                "style": {"backgroundColor":"#12121a","color":"#e0e0e0"}
            }
        )

        st.pydeck_chart(deck)
        st.markdown(
            '<p class="map-caption">🟢 Source → 🟣 Destination | Arc thickness = train frequency</p>',
            unsafe_allow_html=True
        )

        with st.expander("📋 View Route Table"):
            disp = busiest[["source_station","destination_station","count"]].copy()
            disp.columns = ["From","To","Train Count"]
            st.dataframe(disp, use_container_width=True, hide_index=True)

    else:
        st.info("Not enough coordinate data for this selection. Showing bar chart instead.")
        busiest["Route"] = busiest["source_station"] + " → " + busiest["destination_station"]
        st.bar_chart(busiest.set_index("Route")["count"])

    st.divider()

    # ── ⚠️ Delay Risk — Pie + Stacked Bar ────────────────
    st.markdown('<div class="section-title">⚠️ Delay Risk Overview</div>', unsafe_allow_html=True)

    def delay_risk(row):
        if row["stops"] > 15:         return "High"
        if row["distance_km"] > 1000: return "Medium"
        return "Low"
    ...
    with dr2:
        ...
        risk_class = station_df.groupby(["class_type","delay_risk"]).size().reset_index(name="count")
    ...
    # ── 🔮 Price Predictor ───────────────────────────────
    st.markdown('<div class="section-title">🔮 Price Predictor</div>', unsafe_allow_html=True)
    st.markdown('<p style="color:#aaa;">Predict expected fare based on distance and class using ML.</p>', unsafe_allow_html=True)

    @st.cache_resource
    def train_price_model(_data):
        models = {}
        for cls in _data["class_type"].unique():
            sub = _data[_data["class_type"] == cls]
            m   = LinearRegression()
    ...
    with pp2:
        pred_class = st.selectbox("Seat Class", sorted(df["class_type"].unique()), key="pred_class")

    st.markdown('<div class="predict-btn">', unsafe_allow_html=True)
    if st.button("💡 Predict Fare", use_container_width=True):
        predicted = price_models[pred_class].predict([[pred_dist]])[0]
        st.markdown(f'<div class="info-box">💰 Estimated Fare for <b>{pred_dist} km</b> in <b>{pred_class}</b>: <span style="color:#00d9f5;font-size:1.4rem;font-family:Rajdhani,sans-serif;font-weight:700;">₹{predicted:.0f}</span></div>', unsafe_allow_html=True)
        all_preds  = {cls: round(price_models[cls].predict([[pred_dist]])[0], 0) for cls in price_models}
        pred_df    = pd.DataFrame(list(all_preds.items()), columns=["Class","Predicted Fare (₹)"])
        st.table(pred_df)
    ...
    st.markdown('<div class="section-title">🌱 Carbon Footprint Estimator</div>', unsafe_allow_html=True)
    st.markdown('<p style="color:#aaa;">Estimate your CO₂ emissions and compare with car travel.</p>', unsafe_allow_html=True)

    cf1, cf2, cf3 = st.columns(3)
    with cf1:
        all_src_stations = sorted(df["source_station"].unique())
        cf_source = st.selectbox("From Station", all_src_stations, key="cf_src")
    with cf2:
        # ✅ Smart filter: only destinations reachable from selected source
        available_destinations = sorted(
            df[df["source_station"] == cf_source]["destination_station"].unique()
        )
        cf_dest = st.selectbox("To Station", available_destinations, key="cf_dst")
    with cf3:
    ...
    if st.button("🌍 Calculate Carbon Footprint", use_container_width=True):
        route = df[(df["source_station"] == cf_source) & (df["destination_station"] == cf_dest)]
        if route.empty:
    ...
    # ── 📥 Export Train Data ──────────────────────────────
    st.markdown('<div class="section-title">📥 Export Train Data</div>', unsafe_allow_html=True)
    st.markdown('<p style="color:#aaa;">Download filtered train data as a CSV file.</p>', unsafe_allow_html=True)

    exp1, exp2 = st.columns(2)
    with exp1:
        exp_src = st.selectbox("From (optional)", ["All"] + sorted(df["source_station"].unique()), key="exp_src")
    with exp2:
        # ✅ Smart filter: only destinations available from selected source
        if exp_src != "All":
            dst_options = ["All"] + sorted(df[df["source_station"] == exp_src]["destination_station"].unique())
        else:
            dst_options = ["All"] + sorted(df["destination_station"].unique())
        exp_dst = st.selectbox("To (optional)", dst_options, key="exp_dst")

    export_df = df.copy()
    if exp_src != "All": export_df = export_df[export_df["source_station"] == exp_src]
    if exp_dst != "All": export_df = export_df[export_df["destination_station"] == exp_dst]

    st.markdown(f'<p style="color:#a855f7;font-weight:700;">{len(export_df)} records ready to export</p>', unsafe_allow_html=True)
    ...
    st.download_button("⬇️ Download CSV", data=csv_buf.getvalue(),
        file_name="TrainSmart AI_export.csv", mime="text/csv", use_container_width=True)
    st.markdown('</div>', unsafe_allow_html=True)
