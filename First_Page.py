...
def train_model(_data):
    ...
    utility = (X_norm['fare'] * 0.4) + (X_norm['duration_min'] * 0.4) + (X_norm['stops'] * 0.2)
...
@st.cache_data
def build_station_maps(_df):
    src = _df[["source_station","source_code"]].drop_duplicates().set_index("source_station")["source_code"].to_dict()
    dst = _df[["destination_station","destination_code"]].drop_duplicates().set_index("destination_station")["destination_code"].to_dict()

    combined = {**src, **dst}

    name_to_label = {name: f"{name} ({code.strip()})" for name, code in combined.items()}
...
def show_page1():
    ...
    st.markdown('<div class="page-title">🚆 Train Planner</div>', unsafe_allow_html=True)
    ...
    # ─── TRAIN SEARCH ───────────────────────────────────────
    st.markdown('<div class="section-title">🔎 Train Search</div>', unsafe_allow_html=True)
    search = st.text_input("Search by Train Name or Number", placeholder="e.g. Rajdhani or 12301")
    if search:
        res = df[
    ...
    # ─── AI RECOMMENDATION ──────────────────────────────────
    st.markdown('<div class="section-title">🎯 AI Train Recommendation</div>', unsafe_allow_html=True)
    ...
    with c1:
        source_label = st.selectbox("From Station", source_labels, key="from_station")
    ...
    available_dests = sorted(df[df["source_station"] == source]["destination_station"].unique())
    ...
    with c3:
        class_type = st.selectbox("Seat Class", sorted(df["class_type"].unique()))
    ...
    if st.button("🔍 Generate Recommendation", use_container_width=True):
        ...
        if res.empty:
            ...
            similar = df[
                (df["source_station"] == source) | (df["destination_station"] == dest)
    ...
    if "top_train" in st.session_state and st.session_state.top_train:
        ...
        st.markdown('🎟️ Ready to book? &nbsp;[**Book this train on IRCTC →**](https://www.irctc.co.in/nget/train-search)')
        ...
        if not res.empty:
            ...
            st.metric("⭐ Popularity Score", f"{res2.iloc[0]['popularity_score']:.0f}/100")

        def delay_risk(t):
            if t["stops"] > 15:       return "High"
            if t["distance_km"] > 1000: return "Medium"
            return "Low"

        risk       = delay_risk(top)
        risk_class = {"Low":"risk-low","Medium":"risk-medium","High":"risk-high"}[risk]
        risk_icon  = {"Low":"🟢","Medium":"🟡","High":"🔴"}[risk]
        st.markdown(f'<div class="{risk_class}">{risk_icon} Delay Risk: {risk.upper()}</div>', unsafe_allow_html=True)
        st.markdown("")

        st.markdown("**🤖 Why this train?**")
        reasons = []
        if not res.empty:
            if top["fare"]        == res["fare"].min():        reasons.append("✔ Lowest Fare")
            if top["duration_min"]== res["duration_min"].min():reasons.append("✔ Fastest")
            if top["stops"]       == res["stops"].min():       reasons.append("✔ Fewest Stops")
        if not reasons:
            reasons.append("✔ Best overall balance")
        st.markdown("".join([f'<span class="reason-chip">{r}</span>' for r in reasons]), unsafe_allow_html=True)
        st.markdown("")

        # Budget Calculator
        st.markdown('<div class="section-title" style="font-size:1.2rem;">💵 Budget Calculator</div>', unsafe_allow_html=True)
        ...
        # Best Time to Travel
        if not res.empty:
            st.markdown('<div class="section-title" style="font-size:1.2rem;">⏰ Best Time to Travel</div>', unsafe_allow_html=True)
            cheapest = res.loc[res["fare"].idxmin()]
            fastest  = res.loc[res["duration_min"].idxmin()]
            bt1, bt2 = st.columns(2)
            bt1.info(f"💸 **Cheapest Departure:** {cheapest['departure_time']}\nTrain: {cheapest['train_name']}\nFare: ₹{cheapest['fare']:.0f}")
            bt2.info(f"⚡ **Fastest Departure:** {fastest['departure_time']}\nTrain: {fastest['train_name']}\nDuration: {fastest['duration_hours']} hrs")

        st.markdown("")
        save_col, _ = st.columns([1, 3])
        with save_col:
            if st.button("⭐ Save to Favorites"):
                save_favorite(st.session_state.username, top["train_number"], top["train_name"])
                st.success(f"✅ {top['train_name']} saved to favorites!")

        if not res.empty:
            st.markdown('<div class="section-title" style="font-size:1.2rem;">📋 Other Available Trains</div>', unsafe_allow_html=True)
            st.dataframe(
                res[["train_name","train_number","departure_time","arrival_time","fare","duration_hours","stops","score"]].head(10),
    ...
    # ─── TRAIN COMPARISON ───────────────────────────────────
    st.markdown('<div class="section-title">⚖️ Train Comparison</div>', unsafe_allow_html=True)
    ...
    if st.button("🔄 Compare Trains", use_container_width=True):
        ...
        comp = pd.DataFrame({
            ...
            t2: [tr2["fare"], tr2["duration_hours"], tr2["stops"], tr2["distance_km"]]
        })
        st.table(comp)
        st.markdown("**🏆 Category Winners**")
        w1, w2, w3, w4 = st.columns(4)
        w1.metric("💰 Cheaper",      t1 if tr1["fare"]        < tr2["fare"]        else t2)
        w2.metric("⏱ Faster",        t1 if tr1["duration_min"]< tr2["duration_min"]else t2)
        w3.metric("🛑 Fewer Stops",  t1 if tr1["stops"]       < tr2["stops"]       else t2)
        w4.metric("📏 Shorter Route",t1 if tr1["distance_km"] < tr2["distance_km"] else t2)

    st.divider()

    # ─── ROUTE INSIGHTS ─────────────────────────────────────
    st.markdown('<div class="section-title">📍 Route Insights</div>', unsafe_allow_html=True)
    ...
    all_stations      = sorted(set(df["source_station"].unique()) | set(df["destination_station"].unique()))
    all_station_labels= [name_to_label.get(s, s) for s in all_stations]
    sel_label         = st.selectbox("🗺️ Select a Station to Explore", all_station_labels, key="route_insights_station")
    ...
    # ─── FAVORITES ──────────────────────────────────────────
    st.markdown('<div class="section-title">⭐ My Favorite Trains</div>', unsafe_allow_html=True)
    favs = get_favorites(st.session_state.username)
    if favs:
        for f in favs:
            col_fav, col_del = st.columns([5, 1])
            with col_fav:
                st.markdown(f'<div class="card" style="padding:0.6rem 1rem;">🚆 <b>{f[1]}</b> <span style="color:#888;">({f[0]})</span></div>', unsafe_allow_html=True)
            with col_del:
